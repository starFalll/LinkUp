...
async function findLegacyUser(openId) {
  let users = await safeGetCollectionRecords('users', {
    where: { openId },
    limit: 1
  });
  ...
  users = await safeGetCollectionRecords('users', {
...
function mergeCanteenRecord(canteen) {
  const base = DEFAULT_CANTEENS.find((item) => item._id === canteen?._id) ||
    DEFAULT_CANTEENS.find((item) => item._id === canteen) ||
    {};
...
exports.main = async (event) => {
  ...
  let canteen = null;
  canteen = await safeGetDoc('canteens', canteenId);

  canteen = mergeCanteenRecord(canteen || canteenId);
  ...
  const reviewIds = reviewRecords.map((item) => item._id);
  const likeRecords = reviewIds.length
    ? await safeGetCollectionRecords('canteen_review_likes', {
      where: {
        reviewId: _.in(reviewIds)
      },
      limit: 500
    })
    : [];
  ...
  const reviews = reviewRecords.map((review) => {
    const likes = likeRecords.filter((item) => item.reviewId === review._id);
